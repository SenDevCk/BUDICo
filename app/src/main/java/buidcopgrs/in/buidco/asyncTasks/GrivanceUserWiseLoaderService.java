...
public class GrivanceUserWiseLoaderService extends AsyncTask<String,Void, ArrayList<GrivanceUserWiseData>> {
    static GrivanceUserWiseBinder grivanceUserWiseBinder;
    Activity activity;
    private ProgressDialog dialog1;
    private AlertDialog alertDialog;

    public GrivanceUserWiseLoaderService(Activity activity){
    ...
    public GrivanceUserWiseLoaderService() {
    ...
    protected ArrayList<GrivanceUserWiseData> doInBackground(String... strings) {
        ...
        if (Utiilties.isOnline(activity)) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
                result = WebServiceHelper.getGrivanceUserWise(strings);
